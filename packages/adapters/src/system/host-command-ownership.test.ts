 ...
 * of lines down. Every command in between became invisible to this guard. So the
 * scan has to know a string from code. Over-stripping is the one failure mode that would
 * make the guard pass for the wrong reason, so `:` still shields `https://`, and anything
...
function code(src: string): string {
  ...
  // A `/` opens a regex only where a value cannot already have ended. `<` is absent on
  // purpose: `</div>` is far more common in .tsx than a regex compared with `<`.
  const OPENS_VALUE = "(,=:[!&|?{};+-*%~^";

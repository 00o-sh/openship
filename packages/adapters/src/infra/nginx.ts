...
const FORWARD_VARS = `    set $openship_fwd_proto $scheme;
    ...
    if ($http_x_forwarded_proto) {
        set $openship_fwd_proto $http_x_forwarded_proto;
    }
...
`
    ...
    // The real cert is now live for this host, so its placeholder is unreferenced
    // — drop it. AFTER the reload, never before: until the new conf is actually
    // running, OpenResty is still serving the bootstrap paths, and deleting them
    // first would break TLS for the domain on the next unrelated reload.
    if (realCertServed) {

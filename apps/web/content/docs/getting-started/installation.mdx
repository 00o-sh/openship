...
**Compose install** (the default on Linux with Docker) — the edge runs as a container. Headless creates the admin
and registers a `byo`/`free`/`none` domain; put your own TLS-terminating proxy in front, or add a custom domain
afterward from the dashboard's **Domains** tab (the container edge issues its Let's Encrypt cert there):
...
- **Domain** — `--domain-kind`:
  ...
  - `custom` — Openship's edge issues a Let's Encrypt cert for `--hostname`; `--acme-email` and
    `--edge migrate|takeover|cancel` pre-answer the `:80/:443` takeover. **Bare install only** — on the Compose
    stack, add the custom domain from the dashboard's Domains tab (the container edge certs it), or use `--bare`.

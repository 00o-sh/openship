...
const PROMPTS: PromptDef[] = [
  {
    ...
    build: (_args, ref) =>
      [
        ...
        `- GitHub — ${ref("GET", "/api/github/home")} (accounts + repos in one call), browse repos/branches/files (read-only; you cannot create/delete repos via MCP).`,
  ...
  },
  {
    ...
    build: (args, ref) => {
      ...
      return [
        ...
        "",
        `1. Confirm GitHub is connected and find the repo: call ${ref("GET", "/api/github/home")} (returns connection state, accounts, and repos). If you need branches, ${ref("GET", "/api/github/repos/:owner/:repo/branches")}; to peek at config files, ${ref("GET", "/api/github/repos/:owner/:repo/file")}.`,
  ...
  },
  {
    ...
    build: (_args, ref) =>
      [
        ...
        "",
        `1. Open an upload session: ${ref("POST", "/api/projects/folder/session")}. It returns \`upload\` = { url, method, headers } and a sessionId.`,
        "1b. OUT OF BAND — gzip your folder into a tarball and POST the bytes to `upload.url` with the returned headers and Content-Type: application/gzip. Use a plain HTTP client; there is no MCP tool for this.",
        `2. Detect the uploaded source's stack: ${ref("POST", "/api/projects/folder/scan/:sessionId")} (body may be {}). Returns framework, package manager, install/build/start commands, output dir, port.`,
        `3. Create/update the project that carries the build config: ${ref("POST", "/api/projects/ensure")}. Map the scan fields in (framework = the scan's stack id) and set gitProvider:'upload'. Returns the project id.`,

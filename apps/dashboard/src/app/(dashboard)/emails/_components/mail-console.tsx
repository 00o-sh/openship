 ...
 *
 * It lives here rather than inside a `page.tsx` because two routes render it:
 * /emails on the full platform, and `/` in mail view, where the platform
 * dashboard would be a page of projects a mail operator does not keep (see
 * `(dashboard)/page.tsx`). A page file cannot be the shared unit — Next rejects
 * named exports from one, so the component would have to be imported route-from-
 * route.
 *
 * Which of the three surfaces renders (server list · setup wizard · admin panel)
 * stays `resolveMailView()`'s call, from `?serverId=` and the registry, so both
 * routes reach the same answer for the same state.
 */
...
import { FormModalContent } from "./admin/_shared/form-modal-content";
import { MailSetupForm, type SetupRelay } from "./mail-setup-form";
 ...
 *  is installed. Anything else changing here doesn't move the rail. */
const registrySignature = (servers: MailServerListItem[]) =>
  servers.map((s) => `${s.id}:${s.completed ? 1 : 0}`).join(",");

/**
 * The `useSearchParams()` boundary. Both routes that render the console are
 * dynamic already (the dashboard layout reads cookies), but owning the boundary
 * here means neither route shell has to — and neither can forget it.
 */
...
function MailConsoleInner() {
  ...
  // Last registry shape we told the mail rail about, so a plain refetch that
  // changed nothing doesn't make it refetch too.
  const registrySigRef = useRef<string | null>(null);
  ...
  //
  // Merges into the existing query rather than rebuilding it, so switching
  // servers keeps the operator on the tab they were reading — with the mail rail
  // linking straight to `?tab=`, a rebuild meant "click Mailboxes, switch
  // server, land on Overview". Two params are dropped deliberately:
  //   - `force`: passing an id means "show this server's normal view", and every
  //     caller that does so is explicitly leaving the forced wizard.
  //   - `domain`, but only when the server actually changes: a domain belongs to
  //     one mail server, so carrying it over would make the Mailboxes tab query
  //     the previous server's domain on the new one.
  const setServerInUrl = useCallback(
    (serverId: string | null) => {
      ...
      if (next.get("serverId") !== serverId) next.delete("domain");
  ...
  const refreshMailServers = useCallback(async (): Promise<MailServerListItem[]> => {
    try {
      ...
      // rail from "Set up mail" to the tabs in place, or it reads as a failure.
      const sig = registrySignature(servers);
      if (sig !== registrySigRef.current) {
        registrySigRef.current = sig;
        invalidateMailScope();
      }
      return servers;
    } catch {
  ...
  const handleStart = useCallback(
    async (fromStep?: number) => {
      ...
      try {
          ...
          (event: MailSSEEvent) => {
            switch (event.event) {
              ...
              case "error":
                ...
                // Mark the step the failure belongs to. Without this a failure
                // that happens around the step loop (dead executor, state
                // read/write) leaves every step `pending`, so `hasStarted` is
                // false and the view gate drops back to the empty form —
                // swallowing the message we just set (#492).
                if (event.stepId) {
                  setStatus((prev) => {
                    if (!prev) return prev;
                    return {
        ...
        // A rejection here is "setup never started" — the API answered non-2xx
        // before the stream existed. Unwrap the server's own sentence instead of
        // showing `API 502: Bad Gateway` or a raw JSON body (#492).
        if (err instanceof Error && err.name !== "AbortError") {
          setError(getApiErrorMessage(err, t.emails.setup.startFailedFallback));
        }
        setRunning(false);
      }
    },
  ...
  // Openship Mail: the rail names the section, so the page header does too —
  // "Domains", not "Email Server" over a second "Domains" heading inside the tab
  // (which is where that heading used to live; see _lib/mail-section). Overview
  // keeps "Email Server": it's the server's front page.
  ...
  return (
    <PageContainer>
        ...
        {/* An install that never started leaves us on this view with no step to
            show, so the failure gets rendered here — above the form the operator
            is about to resubmit. Previously it was set and never displayed (#492). */}
        {showSetupForm && error && (
          <div className="mb-6">
            <SetupErrorBanner title={t.emails.setup.startFailed} message={error} />
          </div>
        )}
